...
Write-Host "Stopping ForceHub server..."

$remoteRepoArg = ConvertTo-RemoteSingleQuoted -Value $RemoteRepo
$remoteCommand = "cd $remoteRepoArg && { scripts/forcehub.sh stop || true; } && scripts/forcehub.sh status"

& ssh $SshHost $remoteCommand
if ($LASTEXITCODE -ne 0) {
    throw "Failed to stop ForceHub server over SSH."
}

Write-Host "ForceHub server stop requested."
